...
def connection_thread(connection, addr):
    message = connection.recv(1024).decode().strip()
    print_response(message, addr)

    (command, param1, param2) = parse_params(message)
    if command == "REGISTER" and param1 and not param2:
        ...
        if key == KEY:
            ...
            client_handler(connection, client_id, addr)
        else:
            connection.send("ERROR 403 [FORBIDDEN]\n".encode())
            connection.close()

    elif command == "ADMIN" and param1 and not param2:
        (_, key) = message.strip().split(" ")
        if key == KEY:
            admin_handler(connection, addr)
        else:
            connection.send("ERROR 403 [FORBIDDEN]\n".encode())
            connection.close()
    else:
        connection.send("ERROR 400 [BAD REQUEST]\n".encode())
        connection.close()


def hello_client_thread(master):

    while True:
        connection, addr = master.accept()
        threading.Thread(target=connection_thread, args=(connection, addr), daemon=True).start()
...
if __name__=='__main__':
    ...
    master.bind(("", 0))
    master.listen()
    tcp_addr = master.getsockname()

    # UDP listener
    udpThread = threading.Thread(target=udp_discover, args=(tcp_addr[1],), daemon=True)
    # TCP listener
    tcpThread = threading.Thread(target=hello_client_thread, args=(master,), daemon=True)

    udpThread.start()
    tcpThread.start()

    local_ip = get_local_ip()
    print(f"TCP listener initialized in: ({local_ip}, {tcp_addr[1]})")
